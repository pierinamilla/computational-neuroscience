program hh_model_spike
    ...
    real(dp) :: tj_i = 10.0_dp, tj_f = 10.5_dp
    logical :: spike
    character(len=64) :: arg

    if (command_argument_count()>=3) then
            call get_command_argument(1,arg); read(arg,*) J_inj
            call get_command_argument(2,arg); read(arg,*) tj_i
            call get_command_argument(3,arg); read(arg,*) tj_f
    else
            J_inj = 0.0_dp
            tj_i = 10.0_dp
            tj_f = 10.5_dp
    ...
    do i = 0, itmax
        ...
        !Detección de disparo con criterio V>20mV despues del estimulo
        if (t>=tj_f .and. u_new(1) >= 20.0_dp) then
               spike =.true.
        end if
