program hh_model_spike
    ...
    integer :: n_spikes
    real(dp), dimension(10000) :: spike_times
    real(dp) :: isi_sum, f_hz
    ...
    do i = 0, itmax
        ...
        ! Deteccion de pico por cruce ascendente de 0 mV
        if (u(1) < 0.0_dp .and. u_new(1) >= 0.0_dp) then
            if (i - i_prev_spike > 5) then
                n_spikes = n_spikes + 1
                if (n_spikes <= 10000) spike_times(n_spikes) = t
                i_prev_spike = i
            end if
        end if
    ...
    ! Calcular frecuencia a partir de los ISIs, ignorando los primeros 2
    isi_sum = 0.0_dp
    f_hz = 0.0_dp
    if (n_spikes > 3) then
        ! Promedio de ISIs desde el pico 3 en adelante
        do i = 3, n_spikes - 1
            isi_sum = isi_sum + (spike_times(i+1) - spike_times(i))
        end do
        if (n_spikes - 2 > 0) then
            isi_sum = isi_sum / real(n_spikes - 2, dp)
            f_hz = 1000.0_dp / isi_sum
        end if
    end if

    write(*,*) "SPIKES:", n_spikes, " FREQ:", f_hz
