module mod_rk4_thalamic
    ...
    abstract interface
        pure subroutine deriv_iface(t, u, I_inj, du)
            import :: dp
...
contains
    ...
    subroutine rk4_step(f, t, u, I_inj, h, u_new)
        procedure(deriv_iface) :: f
        real(dp), intent(in)  :: t, I_inj, h
        real(dp), intent(in)  :: u(:)
        ...
        call f(t, u, I_inj, k1)
        utmp = u + 0.5_dp * h * k1
        call f(t + 0.5_dp*h, utmp, I_inj, k2)
        utmp = u + 0.5_dp * h * k2
        call f(t + 0.5_dp*h, utmp, I_inj, k3)
        utmp = u + h * k3
        call f(t + h, utmp, I_inj, k4)

        u_new = u + (h/6.0_dp) * (k1 + 2.0_dp*k2 + 2.0_dp*k3 + k4)
    end subroutine rk4_step

end module mod_rk4_thalamic
